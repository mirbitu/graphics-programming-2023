...
void TexturedTerrainApplication::Render()
{
    ...
    //DrawObject(m_terrainPatch, *materials[2], glm::rotate(static_cast<float>(std::numbers::pi) / 2.0f, axis) * glm::scale(glm::vec3(10.0f)));
    //DrawObject(m_terrainPatch, *materials[3], glm::rotate(static_cast<float>(std::numbers::pi), axis) * glm::scale(glm::vec3(10.0f)));
    //DrawObject(m_terrainPatch, *materials[4], glm::rotate((static_cast<float>(std::numbers::pi) * 3.0f) / 2.0f, axis) * glm::scale(glm::vec3(10.0f)));
    // By translating
    DrawObject(m_terrainPatch, *materials[2], glm::translate(glm::vec3(0.0f, 0.0f, -10.0f)) * glm::scale(glm::vec3(10.0f)));
    ...
    DrawObject(m_terrainPatch, *materials[5], glm::translate(glm::vec3(0.0f, -1.0f, -10.0f)) * glm::scale(glm::vec3(10.0f)));
    DrawObject(m_terrainPatch, *materials[5], glm::translate(glm::vec3(-10.0f, -1.0f, 0.0f)) * glm::scale(glm::vec3(10.0f)));
    DrawObject(m_terrainPatch, *materials[5], glm::translate(glm::vec3(-10.0f, -1.0f, -10.0f)) * glm::scale(glm::vec3(10.0f)));
...
void TexturedTerrainApplication::InitializeTextures()
{
    ...
    m_heightMap2 = CreateHeightMap(m_gridX, m_gridY, glm::ivec2(0, -1));
    m_heightMap3 = CreateHeightMap(m_gridX, m_gridY, glm::ivec2(-1, 0));
...
void TexturedTerrainApplication::InitializeMaterials()
{
    ...
    // 04.4
    m_terrainMaterial->SetUniformValue("DirtGrassRange", glm::vec2(-0.18f, 0.005f));
    m_terrainMaterial->SetUniformValue("GrassRockRange", glm::vec2(0.005f, 0.18f));
    ...
    std::shared_ptr<Material> m_waterMaterial = std::make_shared<Material>(waterShaderProgram);
    materials.push_back(m_waterMaterial);
    m_waterMaterial->SetUniformValue("Color", glm::vec4(1.0f, 1.0f, 1.0f, 0.5f));
    m_waterMaterial->SetUniformValue("WaterTexture", waterTexture);
...
std::shared_ptr<Texture2DObject> TexturedTerrainApplication::CreateHeightMap(unsigned int width, unsigned int height, glm::ivec2 coords)
{
    ...
    for (unsigned int j = 0; j < height; ++j)
    {
        for (unsigned int i = 0; i < width; ++i)
        {
            ...
            float z = 0;
            float noise = stb_perlin_fbm_noise3(x, y, 0.0f, 1.9f, 0.5f, 8) * 0.5;
